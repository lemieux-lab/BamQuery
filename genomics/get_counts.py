...
class GetCounts:
	...
	def get_coverage(self, perfect_alignments, bam_files_list, genome_version):
		...
		alignment_information_ribo_path = self.path_to_output_folder_alignments+'Alignments_information_ribo.dic'
	...
	def get_counts_ribo(self, perfect_alignments, bam_files_list):
		...
		ribo_count_path = self.path_to_output_temps_folder+self.name_exp+'_ribo_counts.csv'
		ribo_count_all_alignments_path = self.path_to_output_temps_folder+self.name_exp+'_ribo_count_All_alignments.csv'
		
		exists_ribo_count = os.path.exists(ribo_count_path)
		
		if self.light:
			alignment_information_ribo_path = self.path_to_output_folder_alignments+'Alignments_information_light_ribo.dic'
		else:
			alignment_information_ribo_path = self.path_to_output_folder_alignments+'Alignments_information_ribo.dic'
		
		exists_alignment_information_ribo = os.path.exists(alignment_information_ribo_path)
		
		last_treated_bam_file = os.path.exists(self.path_to_output_folder_alignments+'info_treated_ribo_bam_files.pkl')
		if last_treated_bam_file:

			with open(self.path_to_output_folder_alignments+'info_treated_ribo_bam_files.pkl', 'rb') as fp:
				last_treated_bam_file = pickle.load(fp)

			try:
				with open(self.path_to_output_folder_alignments+'to_ribo_write.pkl', 'rb') as fp:
					to_write = pickle.load(fp)
			except FileNotFoundError:
				to_write = {} 
		else:
		...
		if not exists_ribo_count :
			t_0 = time.time()
			
			if not exists_alignment_information_ribo :
				alignment_information_ribo = copy.deepcopy(perfect_alignments)
			else:
				with open(alignment_information_ribo_path, 'rb') as fp:
			 		alignment_information_ribo = pickle.load(fp)
			...
			keys = alignment_information_ribo.keys()
			...
			for idx, bam_file in enumerate(info_bams):
				...
				if idx > last_treated_bam_file:
					...
					results = pool.map(get_counts_sample.get_counts_sample, bams, keys, values, [True]*len(values))
					...
					for res in results:
						...
						for index, count_align in enumerate(res):
							...
							if index == 0: 
								...
								if len(alignment_information_ribo[key][-1]) == 0:
									alignment_information_ribo[key][-1] = [0]*total_samples

								alignment_information_ribo[key][-1][index-1] = count
					...
					if (idx % 100 == 0) and (idx != 0):
						...
						with open(self.path_to_output_folder_alignments+'info_treated_ribo_bam_files.pkl', 'wb') as f:  
							pickle.dump(idx, f)

						with open(alignment_information_ribo_path, 'wb') as handle:
							pickle.dump(alignment_information_ribo, handle, protocol=pickle.HIGHEST_PROTOCOL)

						with open(self.path_to_output_folder_alignments+'to_ribo_write.pkl', 'wb') as handle:
							pickle.dump(to_write, handle, protocol=pickle.HIGHEST_PROTOCOL)
			...
			df_counts.sort_values(by=['Peptide Type'])
			df_counts.set_index(['Peptide Type', 'Peptide'], inplace=True)
			df_counts.to_csv(ribo_count_path, header=True)
			
			df_alignments.to_csv(ribo_count_all_alignments_path, index=False, header=True)

			self.super_logger.info('Counts Information saved to : %s ', ribo_count_path)
			...
			self.super_logger.info('Count information already collected in the output folder : %s --> Skipping this step!', ribo_count_path)
			df_counts = pd.read_csv(ribo_count_path, index_col=[0,1])
			
			with open(alignment_information_ribo_path, 'rb') as fp:
				alignment_information_ribo = pickle.load(fp)

			df_alignments = pd.read_csv(ribo_count_all_alignments_path, index_col=[0,1])
		...
		return df_counts, alignment_information_ribo, order, df_alignments
	...
	def get_counts(self, perfect_alignments, bam_files_list, overlap):
		...
		if not exists_count :
			...
			for key in keys:
				...
				strand = alignment_information_rna[key][0]
				new_key = peptide+'_'+position+'_'+strand
		...
		return df_counts, alignment_information_rna, order, df_alignments


	def get_counts_(self, perfect_alignments, bam_files_list, overlap):

		times = []
		order = []
		df_counts = pd.DataFrame()

		rna_count_path = self.path_to_output_temps_folder+self.name_exp+'_rna_count.csv'
		rna_count_all_alignments_path = self.path_to_output_temps_folder+self.name_exp+'_rna_count_All_alignments.csv'
		
		exists_rna_count = os.path.exists(rna_count_path)
		
		if self.light:
			alignment_information_rna_path = self.path_to_output_folder_alignments+'Alignments_information_light_rna.dic'
		else:
			alignment_information_rna_path = self.path_to_output_folder_alignments+'Alignments_information_rna.dic'
		
		exists_alignment_information_rna = os.path.exists(alignment_information_rna_path)
		...
		if not exists_rna_count :
			t_0 = time.time()
			
			if not exists_alignment_information_rna :
				alignment_information_rna = copy.deepcopy(perfect_alignments)
			else:
			...
			keys = alignment_information_rna.keys()
			self.super_logger.info('Total MCS mapped : %s ', str(len(keys)))
